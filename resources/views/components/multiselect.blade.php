<div x-data="{ open: false, field: {{ json_encode($value) }} }">
    ...
    <select x-cloak x-show="open || field.length" x-model="field" class="form-multiselect" x-on:click.outside.stop="open = false" multiple @foreach($options() as $key => $value) {{$key}}="{{$value}}" @endforeach>
