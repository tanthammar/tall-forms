<div x-data="{ open: false, field: {{ json_encode($value) }} }">
    ...
    <select x-cloak x-show="open || field.length" x-model="field" class="form-select" x-on:click.away.stop="open = false" multiple @foreach($options() as $key => $value) {{$key}}="{{$value}}" @endforeach>
