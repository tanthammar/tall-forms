...
<div class="tf-repeater-root">
                ...
                <div class="tf-repeater-wrapper-outer" wire:key="{{ md5($repeater->key.$loop->index) }}">
                    ...
                    <div class="tf-repeater-btns-wrapper">
                        @if($repeater->array_sortable)
                            <button type="button" class="tf-repeater-sorter-color" wire:click="arrayMoveUp('{{ $short_parent_name }}', '{{ $key }}')">
                                <x-tall-svg :path="config('tall-forms.arrow-up-icon')" class="tf-repeater-btn-size" />
                            </button>

                            <button type="button" class="tf-repeater-sorter-color" wire:click="arrayMoveDown('{{ $short_parent_name }}', '{{ $key }}')">
                                <x-tall-svg :path="config('tall-forms.arrow-down-icon')" class="tf-repeater-btn-size" />
                            </button>
                        @endif

                        <button type="button" class="tf-repeater-delete-btn" wire:click.prevent="arrayRemove('{{ $short_parent_name }}', '{{ $key }}')">
    ...
    @endif
    <button type="button" class="tf-repeater-add-button" wire:click.prevent="arrayAdd('{{ $short_parent_name }}')" style="width:fit-content">
