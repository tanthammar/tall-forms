<div class="tall-forms-buttons-root">
    <div class="tall-forms-buttons-wrapper">
        @if($showDelete && optional($model)->exists)
            <x-tall-button size="sm" wire:click.prevent="delete" :text="trans(config('tall-forms.delete'))" color="negative"/>
        @endif
        @if($showReset)
            <x-tall-button size="sm" type="reset" wire:click.prevent="resetFormData" :text="trans(config('tall-forms.reset'))" color="warning"/>
        @endif
        @if($showGoBack)
            <x-tall-button size="sm" wire:click.prevent="saveAndGoBack" color="primary">@lang(config('tall-forms.save-go-back'))</x-tall-button>
        @endif
        @if($showSave)
            ...
            <x-tall-button size="sm" type="submit" wire:click.prevent="saveAndStay" wire:loading.attr="disabled" color="positive">
