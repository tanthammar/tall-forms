...
<li class="tall-forms-file-upload-li" wire:key="{{ md5($field->multiple ? $field->name.'.'.$loop->index : $field->name) }}">
    <div>
        {{--Temporary urls are only available for file type image--}}
        @if( \Str::startsWith($file->getMimeType(), 'image') )
            <div class="tall-forms-file-upload-thumb-wrapper">
                <img class="tall-forms-file-upload-thumb-img" src="{{ $file->temporaryUrl() }}" alt="{{ $file->getClientOriginalName() }}"/>
            </div>
        @else
            <div class="tall-forms-file-upload-icon-wrapper">
                @svg(config('tall-forms.file-icon').$fileIcon($file->getMimeType()), "h-4 w-4")
            </div>
        @endif
    </div>
    <div class="flex-1 px-2">{{ $file->getClientOriginalName() }}</div>
    <button type="button" class="tall-forms-file-upload-delete-btn"
